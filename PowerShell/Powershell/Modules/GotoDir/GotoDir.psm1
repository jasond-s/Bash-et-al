$Global:GoDataDirectory = "$([Environment]::GetFolderPath('LocalApplicationData'))\Goto-Dir\"
$Global:GoRememberFileName = "goto-dir-remember-last.txt"
$Global:GoBookmarkFileName = "goto-dir.txt"
...
function gd([string]$key, [string]$selectedPath = "", [switch]$help, [switch]$h, [switch]$add, [switch]$a, [switch]$delete, [switch]$d, [switch]$clear, [switch]$c, [switch]$show, [switch]$s, [switch]$showAll, [switch]$sa, [switch]$last, [switch]$l)
{
    ...
    {
        Write-Host
        Write-Host "Goto Directory Help"
        Write-Host "-----------------"
        Write-Host "Usage:"
        Write-Host "    gd label"
